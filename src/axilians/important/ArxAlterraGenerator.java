...
public class ArxAlterraGenerator extends PlanetGenerator{
    ...
    @Override // Перерисовывает сектора на планете когда игрок какотой захватил
    public void onSectorCaptured(Sector sector){
        sector.planet.reloadMeshAsync();
    }

    @Override // Перерисовывает сектора на планете когда какойто сектор потерян
    public void onSectorLost(Sector sector){
        sector.planet.reloadMeshAsync();
    }

    @Override // Перерисовывает сектора на планете во время сохранения игры
    public void beforeSaveWrite(Sector sector){
        sector.planet.reloadMeshAsync();
    }

    @Override // Говорит движку, что планета светится/эмитирует свет
    ...
    // Проверяет, можно ли запускать миссию с выбранного сектора:
    ...
    // Сектор должен иметь базу (hasBase())
    // Сектор не атакуется (!isAttacked()),
    // Основной блок ядра (core) достаточно большой (size >= 4).
    // ||
    // s.isBeingPlayed() → возвращает true, если сектор сейчас активен (на нём идёт миссия/игра).
    // state.rules.defaultTeam.cores() → получает список всех ядр (cores) на стороне игрока (команды по умолчанию).
    // .contains(b -> b.block.size >= 4) → проверяет, есть ли ядро размера 4 или больше.
    // Вторая часть проверки означает: даже если сектор ещё не имеет «лучшего ядра» (bestCoreType.size < 4), но сейчас на нём идёт игра и у команды есть ядро размера ≥ 4, то сектор можно использовать для запуска миссий.



    @Override // Проверяет, можно ли посадить юнит/запустить миссию на сектор.
    public boolean allowLanding(Sector sector){
        return sector.planet.allowLaunchToNumbered && (sector.hasBase() || sector.near().contains(this::allowNumberedLaunch));
    }   //Условие: планета разрешает запуск, и && сектор имеет базу или || рядом есть подходящий сектор для запуска.

    @Override    // Находит подходящий сектор для запуска миссии:
    public @Nullable Sector findLaunchCandidate(Sector destination, @Nullable Sector selected){
        if(destination.preset == null || !destination.preset.requireUnlock){
            if(selected != null && selected.isNear(destination) && allowNumberedLaunch(selected)){ // Если выбранный сектор близко и подходит → возвращает его.
                return selected;
            }else{
                return destination.near().find(this::allowNumberedLaunch); // Иначе ищет ближайший разрешённый сектор.
            }
        }else{
            return super.findLaunchCandidate(destination, selected);
            // Если сектор требует разблокировки (requireUnlock) → использует стандартный поиск родительского класса.
        }
    }

    @Override // Определяет текст при наведении на заблокированный сектор на глобальной карте.
    public void getLockedText(Sector hovered, StringBuilder out){
        ...
        // Показывает, что для сектора требуется база/ядро для запуска миссии.
        }else{
            super.getLockedText(hovered, out);
        // вызывает базовый метод родителя, который уже умеет формировать стандартный текст блокировки для сектора.
        // То есть, если текущий класс не обрабатывает сектор по своим правилам, используется стандартная логика родителя.
        }
    }

    @Override // Использует функцию rawHeight(position) для генерации рельефа.
    public float getHeight(Vec3 position){
        float height = rawHeight(position);
        return Math.max(height, water);
    // Ограничивает минимальную высоту уровнем воды (Math.max(height, water)), чтобы вода не проваливалась ниже допустимого уровня.
    }

    @Override // Определяет цвет блока на мини-карте планеты:
    public void getColor(Vec3 position, Color out){
        Block block = getBlock(position); // Получает блок по позиции.
        if(block == Blocks.salt) block = Blocks.sand; // Заменяет соль на песок для отображения (чтобы визуально было проще).
        out.set(block.mapColor).a(1f - block.albedo); // Присваивает цвет (mapColor) и прозрачность (1 - albedo).
    }
    ...
    protected void generate(){
        ...
        pass((x, y) -> {
            ...
            float value = Ridged.noise3d(5, v.x, v.y, v.z, 1, 1f / 55f) + rr - rawHeight(v) * 0f;
            float rrscl = rr * 44 - 2;
